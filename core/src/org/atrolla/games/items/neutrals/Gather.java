...
public class Gather extends NeutralItem {
    ...
    public void applyEffect(Round round, GameItems gameItems) {
        if (isUsed() && getPicker().isPresent()) {
            ...
            round.getCharacters().players.stream().forEach(c -> c.coolDownAbility(round.getTime()));
            round.getCharacters().characters.stream().filter(pickerClass::isInstance).forEach(c -> c.teleports(randomCoordinates));
        }
    }
